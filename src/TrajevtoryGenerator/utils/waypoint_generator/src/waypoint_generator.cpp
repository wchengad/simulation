...
// Waypoint task trigger source select: rc or rviz
#define TRIGGER_SOURCE_RC     0
#define TRIGGER_SOURCE_RVIZ   1

#define TRIGGER_SOURCE        TRIGGER_SOURCE_RC
...
void publish_waypoints_vis() {
    ...
    // public all the chosen pose all togethor in the path
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
    ...
    if (waypointSegments.size()) {
        int size_xl = waypointSegments.size();
        ROS_INFO("xl_test: odom_callback: waypointSegments.size() = %d ....\n", size_xl);
...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    ...
    if (waypoint_type == string("circle")) {
        ...
        if (msg->pose.position.z > 0) {
            ...
            if (waypoints.poses.size() >= 1) {
                publish_waypoints_vis();
                // KEY_xl: public the ay points out into traj
                publish_waypoints();
            }
        }
    }
...
#if (TIGGER_SOURCE == TIGGER_SOURCE_RC)
    else{
        waypoints = point();
        publish_waypoints_vis();
        publish_waypoints();
    }
    ROS_INFO("*****Trigger from RC recieved!******");
#endif
}
...
#if (TRIGGER_SOURCE == TRIGGER_SOURCE_RVIZ)
    ros::Subscriber sub3 = n.subscribe("traj_start_trigger", 10, traj_start_trigger_callback);
#endif
#if (TRIGGER_SOURCE == TIRGGER_SOURCE_RC)
  ros::Subscriber sub3 = n.subscribe("rc_trigger", 10, traj_start_trigger_callback);
#endif
